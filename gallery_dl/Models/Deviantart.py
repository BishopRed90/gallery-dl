...
class DeviationMedia(BaseModel):
    ...
    prettyName: str | None = Field(
        validation_alias=AliasChoices("prettyName", AliasPath("media", "prettyName")),
        default=None,
    )
    ...
    def src(self) -> str:
        ...
        if self.types and self.token:
            ...
            view = next(
                view.get("c") for view in self.types if view.get("t") == "fullview"
            )
            if view:
...
class PremiumContent(DeviationMedia):
    ...
    purchased: bool = Field(
        validation_alias=AliasChoices("hasUserPurchased", "has_user_purchased"),
        default=False,
    )
...
class GalleryFolder(BaseModel):
    ...
    parent_name: str | None = Field(
        validation_alias=AliasChoices("parentName", "parent_name"), default=None
    )
    parent_uuid: Annotated[UUID, PlainSerializer(upper_uuid)] | None = Field(
        validation_alias=AliasChoices("parentUuid", "parent_uuid"), default=None
    )
...
class DeviantAuthor(BaseModel):
    ...
    subscribed: bool = Field(
        validation_alias=AliasChoices("isSubscribed", "is_subscribed"), default=False
    )
    watching: bool = Field(
        validation_alias=AliasChoices("isWatching", "is_watching"), default=False
    )
...
class Deviation(BaseModel):
    ...
    tier_access: str = Field(
        validation_alias=AliasChoices("tierAccess", "tier_access"), default="unlocked"
    )
    url: str
    ...
    tags: list[Tag] | list[str] = Field(
        validation_alias=AliasChoices(AliasPath("extended", "tags"), "tags"), default=[]
    )
    ...
    premium_folder: DeviationPremiumFolder | None = Field(
        validation_alias=AliasChoices("premiumFolderData", "premium_folder_data"),
        default=None,
    )
    premium_content: PremiumContent | None = Field(
        validation_alias=AliasChoices(AliasPath("extended", "pcp"), "pcp"), default=None
    )
